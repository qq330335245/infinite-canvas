export default {
    ...
    settingsPanels: {
        ...
        image: { title: "Image settings", quality: "Quality", size: "Size", resolution: "Resolution", align16: "Align to multiples of 16", align16Hint: "Round dimensions up to the next multiple of 16 after input", aspectRatio: "Aspect ratio", transparent: "Transparent background", transparentHint: "Generate an image without a background when supported by the model", count: "Image count", images: "{{count}} images", grokResolutionHint: "Official: 1k / 2k.", grokQualityHint: "quality=low / medium is only supported on grok-imagine-image-2.0 (default medium).", grokRatioHint: "Official ratios include 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3, 2:1, 1:2, 19.5:9, 20:9, and auto." },
        video: { title: "Video settings", quality: "Quality", size: "Size", seconds: "Seconds", resolution: "Resolution", ratio: "Aspect ratio", duration: "Duration", smart: "Smart", output: "Output", generateAudio: "Generate audio", watermark: "Add watermark", adaptive: "Adaptive", autoFromFirstFrame: "Follow first frame when set; otherwise 16:9", grokResolutionHint: "Official: 480p / 720p (default 480p). 1080p only on grok-imagine-video-1.5 for text/image-to-video; reference-to-video is capped at 720p.", grokResolutionHint15: "Official: 1.5 text/image-to-video supports 480p / 720p / 1080p; reference-to-video (reference_images) still max 720p.", grokRatioHint: "Official ratios: 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3. Adaptive follows the first frame, otherwise 16:9.", grokDurationHint: "Official duration is 1–15 seconds.", sizes: { landscape: "Landscape", portrait: "Portrait", square: "Square", widescreen: "Widescreen", tall: "Tall", auto: "Auto" }, ratios: { landscape: "Landscape", portrait: "Portrait", square: "Square", standardLandscape: "Standard landscape", standardPortrait: "Standard portrait", photoLandscape: "3:2 landscape", photoPortrait: "2:3 portrait", cinematic: "Cinematic", adaptive: "Adaptive" } },
    ...
    videoWorkbench: {
        ...
        references: "Reference images",
        firstFrame: "First frame (optional)",
        firstFrameHint: "One image only → Grok image first frame (image-to-video). Mutually exclusive with reference images below.",
        extraReferences: "Reference images",
        extraReferencesHint: "All go to reference_images (never auto-promoted). Mutually exclusive with the first frame — clear it first. Up to 8 images.",
        videoReferences: "Reference videos",
        audioReferences: "Reference audio",
        removeImage: "Remove reference image",
        removeFirstFrame: "Remove first frame",
        ...
        noFirstFrame: "No first frame (optional). Drop or upload one image for the image field; exclusive with references.",
        noGrokImages: "No reference images. These map only to reference_images (not first frame); exclusive with first frame.",
        firstFrameClearsReferences: "Official API forbids image + reference_images together. First frame set; references cleared.",
        referencesClearsFirstFrame: "Official API forbids image + reference_images together. References added; first frame cleared.",
        firstFrameBlocksReferences: "A first frame is set, so references cannot be added. Remove the first frame first, or keep image-to-video only.",
        ...
        labels: { firstFrame: "First frame", reference: "Ref {{index}}" },
        ...
        workflow: "Workflow",
        workflows: { generate: "Generate", edit: "Edit", extend: "Extend" },
        workflowHints: {
            generate: "Text / image / reference-to-video: POST /v1/videos/generations",
            edit: "Modify an existing clip: POST /v1/videos/edits (source video + prompt; official model grok-imagine-video)",
            extend: "Continue from the last frame: POST /v1/videos/extensions (segment 2–10s, default 6)",
        },
        sourceVideo: "Source video",
        sourceVideoHint: "Official video.url: HTTPS or an uploaded local clip",
        noSourceVideo: "Upload the source video to edit or extend",
        sourceVideoRequired: "Upload a source video first",
        extendDuration: "Extension length",
        extendDurationHint: "Official extension duration is 2–10 seconds (not total output length)",
        jobStatus: "Status: {{status}}",
        pollingHint: "Polling Grok2API job progress…",
    },
    canvas: {
